...
$(document).ready(function() {
	...
	ejs.client = ejs.jQueryClient('http://54.235.68.175:9200');
...
function SearchResult(data) {
	...
	if (this._type === "db") {
		...
		this.name = data._source.name;
		if (this.name === null) {
			this.name = data._source.givenName + " " + data._source.familyName;
		}
		this.photoLink = data._source.photoLink;
		if (this.photoLink === null) {
			this.photoLink = "img/emptyContact.png";
		}
		this.phoneNumbers = data._source.phoneNumbers;
		this.emails = data._source.emails;
		this.ims = data._source.ims;
	}

}

function getContactTemplate(data) {
	var title = $("#"+data.id+'_title').html();
	var body = $("#"+data.id+'_body').html();
	var out = {
			'title':title,
			'body':body
	};
...
function AppViewModel() {
	...
	self.showPopOver = function(data, event) {
		...
		var template = getContactTemplate(data); 
		$target.popover({
			animation : true,
			html : true,
			placement : 'right',
	...
	self.localStorageContactuserId = ko.observable();
	...
	self.gcBtnClass = ko.observable('btn');
	self.gcIconClass = ko.observable('icon-plus-sign');
	self.gcBtnText = ko.observable('Connect');
	self.isGoogleContactConnected = ko.observable(false);
	...
	self.gotoPage = function(b) {
		...
		// window.history.pushState('', 'CloudSearch Login', '/index.jsp#'
		// + self.chosenPage());
	};
	// functions
	self.login = function() {
	...
	self.loginGoogleContacts = function(data, event) {
		// debugger;
		if (self.isGoogleContactConnected()) {
			self.logoutGoogleContacts();
			return;
		}
		loginGoogleContacts(null, null, null);
	};
	self.logoutGoogleContacts = function() {
		localStorage.removeItem('gContactuserId');
		self.updateGCdata(null);
	};
	...
	self.updateUserInfo = function(data) {
		...
		self.localStorageContactuserId = localStorage.gContactuserId;
	...
	self.updateGCdata = function(data) {
		var connected = false;
		if (localStorage.gContactuserId != null && localStorage.email != null) {
			connected = true;
		}
		if (connected || data === 'connected') {
			self.gcBtnClass('btn btn-danger');
			self.gcIconClass('icon-remove');
			self.gcBtnText('Disconnect');
			self.isGoogleContactConnected(true);
		} else if (data === 'refresh') {
			self.gcBtnClass('btn btn-info');
			self.gcIconClass('icon-refresh icon-spin');
			self.gcBtnText('Indexing Data...');
			self.isGoogleContactConnected(false);
		} else {
			self.gcBtnClass('btn');
			self.gcIconClass('icon-plus');
			self.gcBtnText('Connect');
			self.isGoogleContactConnected(false);
		}
...
function getSearchResults() {
	...
	// var type = ['gd','db'];
	var r = ejs.Request().size(20).explain(true).indices(indexID).types(type)
			.query(ejs.QueryStringQuery(userQuery || '*'));
